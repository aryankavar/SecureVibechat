...
export const createGroup = functions.https.onCall(async (data, context) => {
  ...
  const memberChecks = await Promise.all(
    allParticipants.map(uid => getDb().collection('users').doc(uid).get())
  );
 ...
 */
export const addGroupMember = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Must be logged in');
  }
  ...
  const adderName = adderDoc.data()?.displayName || 'Someone';
 ...
 */
export const updateGroupInfo = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Must be logged in');
  }
  ...
  if (name !== undefined) updates['groupInfo.name'] = name.trim();
  if (description !== undefined) updates['groupInfo.description'] = description.trim();
  if (avatarUrl !== undefined) updates['groupInfo.avatarUrl'] = avatarUrl;
